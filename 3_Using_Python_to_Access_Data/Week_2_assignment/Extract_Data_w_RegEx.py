...
Sample data:
http://python-data.dr-chuck.net/regex_sum_42.txt
(There are 87 values with a sum=445822)

The other is the actual data you need to process for the assignment.

Actual data:
http://python-data.dr-chuck.net/regex_sum_297290.txt
(There are 86 values and the sum ends with 611)

Make sure to save the file into the same folder as your Python program.
...
looking for a regular expression of '[0-9]+' and then converting the extracted
...
it can sometimes be a fun exercise.
...
until you have completed the homework.
List comprehension is mentioned in Chapter 10 and
the read() method is covered in Chapter 7.
...
def print_text(fname):
    """Extract all numbers in fname.
...
if len(fname) < 1:
    fname = "regex_sum_297290.txt"

print print_text(fname)
