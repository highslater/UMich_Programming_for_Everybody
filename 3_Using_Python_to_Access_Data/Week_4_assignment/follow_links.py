...
and repeat the process a number of times and report the last name you
find.
...
Hint:
The first character of the name of the last page that you will load is: S
...
But frankly with a little effort and patience you can overcome these
attempts to make it a little harder to complete the assignment without
...
def enter_data():
    ...
    data = (
        raw_input('Enter URL: '),
        int(raw_input('Enter count: ')),
        int(raw_input('Enter position: ')),
        raw_input('Enter tag: ')
    )
    return data
...
def find_position(tags, position):
    """Position."""
...
def main():
    """Main."""
    url, count, position, tag = enter_data()
    print "Starting  : ", url
    for i in range(count):
        tags = scrape(url, tag)
        url = find_position(tags, position)
